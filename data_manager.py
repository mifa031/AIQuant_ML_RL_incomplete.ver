...
def build_training_data(prep_data):
    ...
    training_data['high_low_ratio'] = \
        (training_data['high'].values - training_data['low'].values) / \
        training_data['low'].values
    training_data['open_close_ratio'] = \
        (training_data['open'].values - training_data['close'].values) / \
        training_data['close'].values
    training_data['high_open_ratio'] = \
        (training_data['high'].values - training_data['open'].values) / \
        training_data['open'].values
    training_data['low_open_ratio'] = \
        (training_data['low'].values - training_data['open'].values) / \
        training_data['open'].values
