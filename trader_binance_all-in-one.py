...
having_coin_name = "XLM"         #매도포지션이면 보유한 코인 이름을 알려줘야 함

learned_coins = ["EOS","ETH","XRP","XLM","ETC","LTC","TRX","XMR"]
...
if isBuy == True: #현재 학습된 코인을 갖고 있지 않다면(매수 포지션)
    #학습된 코인들 중 매수 확률이 가장 높으면서, 0.5보다 높은 코인을 매수
    isFirst = True
    for coin in learned_coins:
    ...
    if np.max(buy_probs) > 0.5: # 매수하기
        ...
        order = binance.create_market_buy_order('{}/BTC'.format(learned_coins[np.argmax(buy_probs)]), btc_balance/market_buy_price)
        print(order)

else: #현재 학습된 코인을 갖고 있다면(매도 포지션)
    #학습된 코인의 매도 확률을 계산하여, 0.5보다 높으면 매도 후, 다시 매수 포지션 잡기
    model_code = 'integrated'
    ...
    buy_probs = np.array(buy_probs)
    if sell_prob != np.max(buy_probs):
        if np.max(buy_probs) > 0.5: #매도후 매수하기
            #매도 하기
